...
save('ExtractedFeatures_reduced.mat','dataset','dataset_coherence');
...
load('ExtractedFeatures_reduced.mat')
addpath(genpath('matlabGiftiCifti'))
N_Features = 27;
iqm = 2;
...
IQ_Labels = ones([86 3 10])*20;
for i=1:86
    IQ_Labels(i,:,:) = HIQ(i);
end
IQ_Labels = reshape(IQ_Labels, [size(IQ_Labels,1)*size(IQ_Labels,2)*size(IQ_Labels,3) 1]);

Distance = squareform(pdist(d_SVM));
[Y, R, E] = IsoMap(Distance, 'k', 8);
L = Y.coords{10};
L = L';
...
d_NN_train = d_NN_Reduced(sel_subs_train,:);
        ...
        KNNMdl = fitcknn(d_NN_train,IQ_Labels(sel_subs_train),'NumNeighbors',54,'Distance','minkowski');
